...
public class RenewalQueueService {
    ...
    private final AtomicLong requestIdCounter = new AtomicLong(1); // For generating unique request IDs
    ...
    public String processRequest(boolean approve) {
        ...
        saveRequestsToFile();
        // Use polymorphism to get the appropriate message
    ...
    public boolean isEligibleForRenewal(String learnerId) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length != 6) continue; // Expect id,name,email,licenseType,contact,expiryDate
                if (parts[0].equals(learnerId)) {
                    LocalDate expiryDate = LocalDate.parse(parts[5]); // expiryDate is the 6th field
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    String[] parts = line.split(",");
                    RenewalRequest request;
                    if (parts.length == 3) { // Old format: learnerId,requestDate,isFirstTime
                    ...
                    } else if (parts.length == 5) { // New format: requestId,learnerId,requestDate,isFirstTime,status
